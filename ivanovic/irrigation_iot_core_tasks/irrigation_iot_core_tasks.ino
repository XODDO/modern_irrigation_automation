...
WiFi_Manager wifi_obj(Pins::WIFI_LED);
upload Uploader(Pins::SERVER_LED, Netwaka::SERVER_URL);

// Reserve enough JSON heap for parsing incoming payloads. 4KB is large, but ok here.
static StaticJsonDocument<Memory::JSON_BUFFER_SIZE> JSON_received;


// --- MUTEX and BUFFERS for double-buffered UART/upload ---
SemaphoreHandle_t xDataMutex = NULL;
char receiveBuffer[Memory::JSON_BUFFER_SIZE];
char uploadBuffer[Memory::JSON_BUFFER_SIZE];

// Instead of:
// constexpr int32_t wiot_upload_frequency = int32_t(10 * 60 * 1000);
// Use:
constexpr int32_t wiot_upload_frequency = Netwaka::UPLOAD_FREQUENCY_MS;
...
char upload_logs[Memory::UPLOAD_LOG_CAPACITY][150];
// --- OTA logging ---
char ota_log[150] = "...";
...
//   uint8_t status_code = SystemStatus::BOOT;
SystemStatus status_code = SystemStatus::BOOT;
...
void setup() {
    ...
    Serial.begin(115200); 
    delay(500); 
    status_code = SystemStatus::BOOT; 
        log_status(status_code);  // No need for static_cast

    Serial2.begin(115200, SERIAL_8N1, 16, 17); 
    status_code = SystemStatus::SERIAL_INIT;;
    Serial2.setRxBufferSize(Memory::JSON_BUFFER_SIZE);  
    status_code = SystemStatus::BUFFER_READY;      
    log_status(status_code);

    pinMode(Pins::HEARTBEAT_LED, OUTPUT); digitalWrite(Pins::HEARTBEAT_LED, HIGH); delay(500);
    pinMode(Pins::INTERNAL_FAN, OUTPUT); digitalWrite(Pins::INTERNAL_FAN,  HIGH); delay(500);
    pinMode(Pins::FLOW_METER_LED, OUTPUT); digitalWrite(Pins::FLOW_METER_LED,  HIGH); delay(500);
    pinMode(Pins::SERVER_LED, OUTPUT); digitalWrite(Pins::SERVER_LED, HIGH); delay(500);

    
    // status_code = static_cast<uint8_t>(wifi_obj.initialize_ESP_WiFi(Netwaka::DEVICE_NAME));
    uint8_t wifi_status = wifi_obj.initialize_ESP_WiFi(Netwaka::DEVICE_NAME);
    if (wifi_status >= static_cast<uint8_t>(SystemStatus::BOOT) && 
        wifi_status <= static_cast<uint8_t>(SystemStatus::WIFI_FAIL)) {
        status_code = static_cast<SystemStatus>(wifi_status);
    } else {
        status_code = SystemStatus::WIFI_FAIL; // Default to failure
        Serial.printf("[WARN] Unknown WiFi status: %u\n", wifi_status);
    }
    log_status(status_code);
    ...
    Serial.println(Netwaka::DEVICE_NAME);

    digitalWrite(Pins::HEARTBEAT_LED, LOW); delay(500);
    digitalWrite(Pins::WIFI_LED, LOW);  delay(500);
    digitalWrite(Pins::SERVER_LED, LOW); delay(500);
    digitalWrite(Pins::FLOW_METER_LED, LOW); delay(500);
    ...
    xTaskCreatePinnedToCore(fanning_task, "InternalTEMP_MGT", TaskStack::FANNING, NULL, TaskPriority::PRIORITY_LOW, NULL, 1);

    xTaskCreatePinnedToCore(Serial_RX_Task, "Update_RECV_BUFF", TaskStack::SERIAL_RX, NULL, TaskPriority::AVG, NULL, 1);
    ...
    BaseType_t RC = xTaskCreatePinnedToCore(upload_task, "Upload to the Internet", TaskStack::UPLOAD, NULL, TaskPriority::PRIORITY_HIGH, NULL, 0); // core 0
    ...
    digitalWrite(Pins::INTERNAL_FAN, LOW); delay(500);
}

void Serial_RX_Task(void *pvParams) {
    // Validate mutex exists  
    if (xDataMutex == NULL) {
        Serial.println("[Serial_RX_Task] ERROR: Mutex not available!");
        vTaskDelete(NULL);
    }

    const size_t task_stack_size = TaskStack::SERIAL_RX;  // matches xTaskCreatePinnedToCore allocation
...
void upload_task(void *pvParams) {
      // Validate mutex exists
    if (xDataMutex == NULL) {
        Serial.println("[upload_task] ERROR: Mutex not available!");
        vTaskDelete(NULL); // Suicide if no mutex
    }

    TickType_t prev_wake_time = xTaskGetTickCount();
    char localJson[Memory::JSON_BUFFER_SIZE];
    ...
    while (true) {
        ...
        localJson[0] = '\0';
            ...
            if (!wifi_connected) { // Extra debug: WiFi status before trying upload
                ...
                status_code = SystemStatus::WIFI_FAIL;
        ...
        vTaskDelayUntil(&prev_wake_time, pdMS_TO_TICKS(Netwaka::UPLOAD_FREQUENCY_MS)); //  ; wiot_upload_frequency
    }
...
void fanning_task(void *pvParams) {
    ...
    UBaseType_t uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
    const size_t task_stack_size = TaskStack::FANNING; // same as you allocated
...
void loop() {
    ...
    if (!receiving_UART) {
            flash(now_now, Pins::HEARTBEAT_LED, 50, 75, 50, 1500); 
         ...
         // UPLOAD IS HANDLED BY RTOS TASK
          //  printSystemDiagnostics(now_now); // only print when connected to PC
             ...
             flash(now_now, Pins::HEARTBEAT_LED, 200, 1000, 0, 0);        //          flash(now_now, wifi_led, 100, 100, 100, 900);
...
void initializeOTA() {
    ArduinoOTA.setHostname(Netwaka::DEVICE_NAME);
    ArduinoOTA.setPassword(Netwaka::OTA_PASSWORD);
...
void log_status(SystemStatus state) {
    char details[200] = ".";
    switch (state) {
        case SystemStatus::BOOT:
            snprintf(details, sizeof(details), "%s Booting...", Netwaka::DEVICE_NAME);
            break;
        case SystemStatus::SERIAL_INIT:
            snprintf(details, sizeof(details), "Serial2 initialized");
            break;
        case SystemStatus::BUFFER_READY:
            snprintf(details, sizeof(details), "Serial2 buffer set to %d bytes", Memory::JSON_BUFFER_SIZE);
            break;
        case SystemStatus::WIFI_INIT:
            snprintf(details, sizeof(details), "WiFi initialized");
            break;
        case SystemStatus::RUNNING:
            snprintf(details, sizeof(details), "System running");
            break;
        case SystemStatus::WIFI_FAIL:
...
void monitor_internal_temp() {
    ...
    if (cabin_temp >= 33.0 && !inner_fan_status) {
        inner_fan_status = true; digitalWrite(Pins::INTERNAL_FAN, HIGH);
    } else if (cabin_temp <= 30.0 && inner_fan_status) {
        inner_fan_status = false; digitalWrite(Pins::INTERNAL_FAN, LOW);
    }
...
void printSystemDiagnostics(uint64_t now_diag_time) {
    ...
    if (xSemaphoreTake(xDataMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
            ...
            size_t copyLen = strnlen(uploadBuffer, Memory::JSON_BUFFER_SIZE);
...
void receive_serial_data(uint64_t call_time) {
    ...
    while (Serial2.available()) {
        ...
        if (braceLevel > 0) {
            if (bufferIndex < Memory::JSON_BUFFER_SIZE - 1)
            ...
            if (braceLevel == 0 && bufferIndex > 0) {
                ...
                if (!error) {
                  ...
                  //  Serial.println(receiveBuffer); // print to screen only for diagnostics
                    ...
                    cabin_temp = JSON_received[System::JsonFields::INTERNAL_TEMP] | 0.0;
                    ...
                    if (xSemaphoreTake(xDataMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
                        strncpy(uploadBuffer, receiveBuffer, Memory::JSON_BUFFER_SIZE);
                        uploadBuffer[Memory::JSON_BUFFER_SIZE-1] = '\0'; // // Ensure null termination
                        xSemaphoreGive(xDataMutex);
                    }
                } else {
...
void receive_serial_data(){
  ...
  while (Serial2.available() && Serial2.read() != '}') { // drain until the next {, so you can desync if noise appears.
    ...
    // Start collecting once '{' is seen
        ...
        if (braceLevel > 0) {
            if (bufferIndex < Memory::JSON_BUFFER_SIZE - 1) { // leave space for null terminator
