...
void Battery::update() {
  ...
  _voltage = _cells * (avg * _scaleFactor / 3196.0f);  

    // if _scaleFactor == 14.4 >> 1 Lead Acid Batteries
    // if _scaleFactor = 8.4 >> 2 Li ion Cells

  // Voltage-based levels
  if (_voltage <= 3.6f) { // < 3.5V POWER_CRITICAL // standard lithium polymer (LiPo) cell's lowest allowed voltage is typically 3.0V, per cell to avoid irreversible damage
    _level = 0;
  } else if (_voltage <= 3.75f) { // <= 3.8V POWER_LOW 
    _level = 1;
  } else if (_voltage <= 3.95f) { // <= 3.9V POWER_MODERATE
    _level = 2;
