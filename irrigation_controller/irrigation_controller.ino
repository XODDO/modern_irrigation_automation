...
void flash(uint64_t now_now, uint8_t heartbeat,
...
void setup() {
  ...
  // DISPLAY
      ...
      pinMode(heartbeatLED, OUTPUT);  digitalWrite(heartbeatLED, HIGH); delay(500);

      pinMode(sensor_1_active_pin, OUTPUT); digitalWrite(sensor_1_active_pin, HIGH); delay(500);
      pinMode(sensor_2_active_pin, OUTPUT); digitalWrite(sensor_2_active_pin, HIGH); delay(500);
      pinMode(irrigation_valve_pin, OUTPUT); digitalWrite(irrigation_valve_pin, HIGH); delay(500);
 ...
 //   xTaskCreatePinnedToCore(ScreenTask, "UpdateDisplay", 4096, NULL, PRIORITY_NORMAL, NULL, 1);
    ...
    Serial.println("\tIrrigation Management Task initialized!");

     currentScreen = 1; update_display(); vTaskDelay(pdMS_TO_TICKS(500));

    Serial.println("Started in ESPNOW MODE - NORMAL MODE");

  // Initialize ESPNOW immediately
   switch_radio_to_espnow();
    vTaskDelay(pdMS_TO_TICKS(2000));
    digitalWrite(heartbeatLED, LOW); vTaskDelay(pdMS_TO_TICKS(500));
    digitalWrite(irrigation_valve_pin, LOW); vTaskDelay(pdMS_TO_TICKS(500));  // turn OFF after init
 ...
 //after establishing power state
  ...
  //  digitalWrite(indicator, LOW); vTaskDelay(pdMS_TO_TICKS(500)); 
    digitalWrite(sensor_1_active_pin, LOW); vTaskDelay(pdMS_TO_TICKS(500));
    digitalWrite(sensor_2_active_pin, LOW); vTaskDelay(pdMS_TO_TICKS(500));
    // turn OFF after init
...
int valve_checkin_frequency = (15*1000); // default every 30 seconds
void ManageIrrigation(void * pvParams) { 
  valve_checkin_frequency = is_irrigating? 1000 : 30000; // check more often when irrigating
    while (true) {
        ...
        now_now = esp_timer_get_time(); // current time in microseconds
...
float discharge = 0.0f; // flow rate in L/min
...
void loop() {
  ...
  if((now_now-last_loop_time) >= (2*60*1000ULL)){ // once every 2 minutes
    counter++;
    if(!clock_is_working) uptime(now_now);
     ...
     send_to_wifi_core();
     update_display();
    if(counter >= 2)  { uploaded = true; counter = 0; }
    else uploaded = false;
    
    last_loop_time = now_now;
  }
  ...
  if (read_button(ota_button, now_now) && !otaModeActive) {
    ...
    if (wifi_connected) {
      initializeOTA();
      Serial.println("OTA Mode Active for 5 minutes...");
    }
    else {
  ...
  if (otaModeActive) {
    ...
    if (now_now - otaStartTime >= OTA_TIMEOUT) {
      Serial.println("OTA timeout reached — returning to ESP-NOW mode...");
      otaModeActive = false;
      switch_radio_to_espnow();
    }
  } // !otaModeActive
    ...
    flash(now_now, heartbeatLED, 50, 75, 50, 1500);
